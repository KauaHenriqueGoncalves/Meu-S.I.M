...
import com.meusim.application.modules.identity.base.role.service.RoleService;
...
public class UserServiceImpl implements UserService {
        ...
        return userRepository.findForLogin(email, schoolCode)
                .orElseThrow(() -> {
                    log.warn("Tentativa de login com credenciais inválidas. [email={}] [schoolCode={}]",
                            email, schoolCode);
                    return new NotFoundObjectException("Não encontrou usuário");
                });
    }

    @Override
    @Transactional
    public User registerUserWithRole(CreateUserRequestDTO dto, Role.Values roleValues) {
        log.info("Iniciando cadastro de usuário. [email={}] [cpf={}] [perfil={}]",
                dto.email(), dto.cpf(), roleValues.name());

        checkUserAlreadyExists(dto);
        checkUserConflict(dto);
    ...
    private void checkUserAlreadyExists(CreateUserRequestDTO request) {
    ...
    private void checkUserConflict(CreateUserRequestDTO request) {
