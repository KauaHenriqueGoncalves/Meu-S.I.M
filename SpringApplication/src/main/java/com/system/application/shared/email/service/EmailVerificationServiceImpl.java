...
public class EmailVerificationServiceImpl implements EmailVerificationService{
    ...
    public String createToken(User user) {
        Optional<EmailVerificationToken> existing = emailVerificationRepository.findByUserId(user.getId());
    ...
    public void validateUser(String token) throws NotFoundObjectException, AccessDeniedException {
        EmailVerificationToken verificationToken = emailVerificationRepository.findByToken(token).orElseThrow(
                () -> new NotFoundObjectException("Token invalido")
        );
        ...
        User user = userRepository.findById(verificationToken.getUserId()).orElseThrow(
                () -> new NotFoundObjectException("Usuario não existe no token")
        );
        user.setActive(true);
